...
function startCrawlJob(source, maxDepth, maxPages) {
    ...
    db.prepare("UPDATE sources SET crawl_depth=?, max_pages=? WHERE id=?").run(maxDepth, maxPages, source.id);
...
function pageCountFor(sourceId) {
    return db.prepare("SELECT COUNT(*) as c FROM pages WHERE source_id = ?").get(sourceId).c;
}
// --- Tool definitions ---
const tools = [
    {
    ...
    {
        name: "delete_source",
        description: "Remove a documentation source and all its indexed pages from the database.",
        inputSchema: {
            type: "object",
            properties: {
...
// --- Handlers ---
function handleAddDocsUrl(args) {
    const { url, max_depth, max_pages } = AddDocsUrlSchema.parse(args);
    const normalized = normalizeUrl(url);
    let source = db.prepare("SELECT * FROM sources WHERE url = ?").get(normalized);
    if (!source) {
        db.prepare("INSERT INTO sources (url, added_at, crawl_depth, max_pages, status) VALUES (?, ?, ?, ?, ?)").run(normalized, Date.now(), max_depth, max_pages, "pending");
        source = db.prepare("SELECT * FROM sources WHERE url = ?").get(normalized);
    }
...
function handleSearchDocs(args) {
    ...
    for (const s of allSources) {
        if (s.status === "ready" && isStale(s))
            staleSources.push(s.url);
    }
    if (pages.length === 0) {
        return {
...
function handleGetPage(args) {
    ...
    const meta = db
        ...
        .get(normalized);
    if (!meta)
        return { error: `Not found: ${normalized}. It may not have been crawled yet.` };
    ...
    const row = db
        ...
        .get(offset + 1, chunk_size, meta.id);
...
function handleExpandTopic(args) {
    ...
    if (source_url) {
        ...
        const src = db.prepare("SELECT id FROM sources WHERE url = ?").get(normalizeUrl(source_url));
        if (src) {
            pages = db
                .prepare(`SELECT DISTINCT ${cols} FROM pages p
    ...
    return {
        ...
        pages: pages.map((p) => {
           ...
           END as snippet
           FROM pages WHERE id = ?`)
                .get(topicLower, topicLower, p.id);
            return {
                url: p.url,
                title: p.title,
...
function handleRecrawlSource(args) {
    const { url } = UrlSchema.parse(args);
    const normalized = normalizeUrl(url);
    const source = db.prepare("SELECT * FROM sources WHERE url = ?").get(normalized);
    if (!source)
        return { error: `Not found: ${normalized}. Add it first with add_docs_url.` };
...
function handleGetCrawlStatus(args) {
    const { url } = UrlSchema.parse(args);
    const normalized = normalizeUrl(url);
    const source = db.prepare("SELECT * FROM sources WHERE url = ?").get(normalized);
    if (!source)
        return { error: `Not found: ${normalized}.` };
...
function handleDeleteSource(args) {
    const { url } = UrlSchema.parse(args);
    const normalized = normalizeUrl(url);
    const source = db.prepare("SELECT * FROM sources WHERE url = ?").get(normalized);
    if (!source)
...
// --- MCP Server ---
const server = new Server({ name: "rock-mcp", version: "0.5.0" }, { capabilities: { tools: {}, resources: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        switch (name) {
            case "add_docs_url":
                result = handleAddDocsUrl(args);
                break;
            case "search_docs":
                result = handleSearchDocs(args);
                break;
            case "get_page":
                result = handleGetPage(args);
                break;
            case "expand_topic":
                result = handleExpandTopic(args);
                break;
            case "list_sources":
                result = handleListSources();
                break;
            case "recrawl_source":
                result = handleRecrawlSource(args);
                break;
            case "get_crawl_status":
                result = handleGetCrawlStatus(args);
                break;
            case "delete_source":
                result = handleDeleteSource(args);
                break;
            case "search_web":
...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
    const sources = db.prepare("SELECT * FROM sources WHERE status = 'ready'").all();
    return {
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    if (!match)
        return { contents: [{ uri, mimeType: "text/plain", text: "Invalid URI" }] };
    const sourceUrl = decodeURIComponent(match[1]);
    const source = db.prepare("SELECT * FROM sources WHERE url = ?").get(sourceUrl);
    if (!source)
        return { contents: [{ uri, mimeType: "text/plain", text: "Source not found" }] };
    const pages = db
